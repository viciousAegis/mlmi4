...
def get_cifar10_loaders(
    ...
    """
    train_full = datasets.CIFAR10(root=root, train=True, download=True, transform=_make_transform(train=True))
    test_set   = datasets.CIFAR10(root=root, train=False, download=True, transform=_make_transform(train=False))

    if not (0 < val_size < len(train_full)):
        raise ValueError(f"val_size must be between 1 and {len(train_full)-1}, got {val_size}")

    gen = torch.Generator().manual_seed(seed)
    train_set, val_set = random_split(train_full, [len(train_full) - val_size, val_size], generator=gen)
    ...
    val_base = datasets.CIFAR10(root=root, train=True, download=False, transform=_make_transform(train=False))
